...
// Test configuration
const GOTTY_URL = 'http://192.168.1.242:8888';
...
async function testS9sTUI() {
  ...
  try {
    console.log('1. Navigating to gotty...');
    await page.goto(GOTTY_URL);
    await page.waitForLoadState('networkidle');
    await sleep(2000); // Wait for terminal to initialize
    
    // The gotty terminal is inside the body element
    ...
    await page.screenshot({ path: 'test/e2e/s9s-gotty-test.png', fullPage: true });
    ...
    await page.screenshot({ path: 'test/e2e/s9s-gotty-error.png', fullPage: true });
    throw error;
  } finally {
    await browser.close();
  }
